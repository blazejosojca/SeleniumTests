...
class SanityPageTests(MainTest):
    ...
    def test_02_check_search_engine(self):
        ...
        searching_icon = self.driver.find_element_by_css_selector(HomePageLocators.CSS_SEARCH_ICON)
        searching_icon.click()
        oh.visibility_of_element_wait(self.driver, 'css', 'section[id=main] h2.h2')
        list_of_elements = self.driver.find_elements_by_css_selector('h1[itemprop="name"]')
        found_elements = 0
    ...
    def test_03_check_currency_of_products(self):
        ...
        home_page = HomePage(self.driver)
        home_page.driver.get(HomePage.main_url)
        prices_elements = self.driver.find_elements_by_css_selector(HomePageLocators.CSS_PRICE_ELEMENTS)
